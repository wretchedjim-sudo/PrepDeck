...
export default function App() {
  ...
  function buildInterviewPrompt(asset: string) {
    const guestInformation = {
      guestName: interviewDraft.guestName,
      guestTitle: interviewDraft.guestTitle,
      organization: interviewDraft.organization,
      interviewTopic: interviewDraft.topic,
      highPointsMainAngle: interviewDraft.reason,
      guestBackgroundWhatWeKnow: interviewDraft.bio,
      contactProductionNotes: interviewDraft.contact,
      audience: interviewDraft.audience,
      segmentLength: interviewDraft.segmentLength,
      tone: interviewDraft.tone,
      mustAsk: interviewDraft.mustAsk,
      avoid: interviewDraft.avoid,
    };
...
These must be specific to the guest information. Do not write questions that could fit any guest.

Hard rules:
- Do not open with "tell us who you are."
- Do not ask "why does this matter?" unless you tie it to a specific detail from the form.
- Do not ask "what do you wish people would ask?"
- Do not ask generic podcast questions.
- Every question must be connected to a concrete detail, relationship, event, role, history, location, organization, listener interest, or situation from the form.
- If the guest is a former host, former employee, returning local figure, longtime community voice, or someone listeners already know, lean into that history.
- Build questions that create warmth, memory, curiosity, story, and listener connection.
- Include questions that help spotlight what the guest is doing now.
- Include a few questions that only a prepared local host would ask.
- Include a few surprising questions that are still respectful and usable on air.

Output format:
1. Opening / Warm Questions
2. Main Conversation Questions
3. Outside-the-Box Questions
4. Listener-Connection Questions`,

      "Follow-Up Questions": `
Create 15 follow-up questions.

These are not standalone generic questions. These are smart second-level questions a host can use after the guest answers.

Hard rules:
- Each follow-up must push for a better story, clearer detail, stronger emotion, better memory, or more useful listener takeaway.
- Avoid filler.
- Avoid "can you expand on that?" unless you rewrite it into a specific follow-up.
- Make the host sound like he is listening closely.
- Use the guest information to predict where the conversation should go.

Output format:
1. Story Follow-Ups
2. Detail Follow-Ups
3. Listener Follow-Ups
4. If the Guest Gives a Short Answer`,

      "Guest Intro": `
Write a detailed guest introduction for a radio host.

Hard rules:
- Use the guest background heavily.
- Do not write a generic guest intro.
- Make it sound like the host actually knows this person and why listeners care.
- If the guest has a history with the station, the town, the audience, or the show, make that central.
- Include what the guest is doing now.
- Keep it natural enough to read out loud.

Output:
1. Full Host Intro
2. Shorter Reset Intro
3. One-Sentence Guest Setup`,

      "Segment Tease": `
Write 8 segment teases.

Hard rules:
- These should sound like radio, not a podcast description.
- Each tease should create curiosity.
- Use the actual guest and topic details.
- If the guest has a history with listeners, use that.
- Include a warm tease, a curiosity tease, a nostalgic tease, a direct tease, and a quick rejoin tease.`,

      "If Time Allows Questions": `
Create 10 bonus questions.

Hard rules:
- These should not be leftovers.
- They should be useful when the interview is going well.
- Include a few lighter questions, a few reflective questions, and a few current-life questions.
- Use the guest information.
- Do not write generic filler.`,
    };

    return `You are the senior producer for a live local radio morning show.

You are not a chatbot. You are not writing generic interview prep. You are preparing a host to do a strong, warm, specific, interesting live radio interview.

The host has filled out this interview prep form. Your job is to study the details and create ${asset} that could only belong to this guest and this segment.

GUEST FORM DATA:
${JSON.stringify(guestInformation, null, 2)}

TASK:
${assetInstructions[asset] || "Create a specific radio prep asset based on the form data."}

IMPORTANT:
If the form says the guest is a former morning show host, former station personality, returning local figure, or someone the audience already knows, do not treat him like a random guest. Build the prep around:
- the listeners remembering him
- what it feels like to be back
- what has changed since he left
- what he misses
- what he does now
- old memories without sounding cheesy
- the handoff from his old role to the current host
- why this is a feel-good catch-up segment

BANNED GENERIC QUESTIONS:
- "Tell us about yourself."
- "Why does this matter?"
- "What do you wish people knew?"
- "What inspired you?"
- "What is your message to listeners?"
- "Where can people find you?" unless it is placed at the end as a practical close.

Write like a real radio producer who wants the host to sound sharp, prepared, local, and human.`;
  }
  ...
  async function generateInterviewAsset(asset: string) {
    ...
    setInterviewOutput("");
    setInterviewStatus("Generating with local AI...");
    ...
    try {
      ...
      setInterviewOutput(cleaned);
      setInterviewStatus("");
    } catch (error) {
      console.error("PrepDeck local AI failed:", error);
      setInterviewOutput("");
      setInterviewStatus(
        typeof error === "string"
          ? error
          : "Local AI did not return a usable response."
      );
    } finally {
      ...
      <main className="app-shell">
        ...
        <section className="interview-layout">
          ...
          <aside className="interview-output-panel">
            ...
            {interviewStatus ? (
              <p className="status-line">{interviewStatus}</p>
            ) : null}
